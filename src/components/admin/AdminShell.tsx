...
export default function AdminShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] flex">
      <aside className="fixed top-0 left-0 h-full w-64 bg-[#0f0f0f] border-r border-white/[0.05] flex flex-col z-50">
        <div className="px-5 py-5 border-b border-white/[0.05]">
          <Link href="/admin" className="flex items-center gap-3 group">
            ...
            <div>
              <p className="text-white font-bold text-sm tracking-[0.08em] uppercase leading-none" style={{ fontFamily: "Satoshi, sans-serif" }}>
                Movico
              </p>
              <p className="text-white/30 text-[10px] tracking-[0.12em] uppercase mt-0.5">CMS</p>
            </div>
          </Link>
        </div>
      ...
      <main className="ml-64 flex-1 min-h-screen overflow-x-hidden">{children}</main>
    </div>
  );
}
